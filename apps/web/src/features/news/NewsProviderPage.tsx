...
export function NewsProviderPage({ onChanged }: { onChanged?: () => void }) {
  ...
  return (
    <section className="news-workspace">
      ...
      {tab === "PROVIDER" ? <section className="settings-card"><div className="settings-card__title"><CloudDownload size={18}/> Proveedor y sincronización</div><div className="settings-form-grid"><label>Proveedor<select value={settings.provider} onChange={(e) => update("provider", e.target.value as NewsSettings["provider"])}><option value="TORUM">Torum automático</option><option value="MANUAL">Manual</option></select></label><label className="toggle-line"><input checked={settings.provider_enabled} type="checkbox" onChange={(e) => update("provider_enabled", e.target.checked)}/>Proveedor activo</label><label className="toggle-line"><input checked={settings.auto_sync_enabled} type="checkbox" onChange={(e) => update("auto_sync_enabled", e.target.checked)}/>Sincronización automática</label><label>Intervalo (min)<input min="15" max="10080" type="number" value={settings.sync_interval_minutes} onChange={(e) => update("sync_interval_minutes", Number(e.target.value))}/></label><label>Días futuros<input min="1" max="90" type="number" value={settings.days_ahead} onChange={(e) => update("days_ahead", Number(e.target.value))}/></label></div><p className="settings-helper-text"><strong>Torum automático</strong> usa BLS, BEA, Reserva Federal y Census sin claves. FMP es un complemento opcional: una única API key puede añadir previsiones, valores previos/actuales y eventos macro adicionales.</p><dl className="metric-list"><div><dt>Estado</dt><dd>{provider?.last_sync_status ?? "--"}</dd></div><div><dt>Aviso / error</dt><dd>{provider?.last_sync_error ?? "Ninguno"}</dd></div></dl></section> : null}
