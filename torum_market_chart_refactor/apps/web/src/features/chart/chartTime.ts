...
export function normalizeUnixSeconds(value: unknown): UTCTimestamp | null {
  ...
  if (typeof value === "string") {
    ...
    if (!Number.isNaN(parsedDate)) return Math.floor(parsedDate / 1000) as UTCTimestamp;
  }
...
export function readChartBrokerTimeZone(): string {
  ...
  if (typeof window === "undefined") return validTimeZoneOrFallback(envValue, defaultChartBrokerTimeZone);
...
const chartTickYearFormatter = new Intl.DateTimeFormat("es-ES", { timeZone: chartDisplayTimeZone, year: "numeric" });
const chartTickMonthFormatter = new Intl.DateTimeFormat("es-ES", { month: "short", timeZone: chartDisplayTimeZone });
const chartTickDayFormatter = new Intl.DateTimeFormat("es-ES", { day: "2-digit", month: "2-digit", timeZone: chartDisplayTimeZone });
...
export function timeZoneOffsetSeconds(formatter: Intl.DateTimeFormat, utcUnixSeconds: number): number {
  ...
  const localAsUtcSeconds = Math.floor(
    Date.UTC(values.year ?? 1970, (values.month ?? 1) - 1, values.day ?? 1, hour ?? 0, values.minute ?? 0, values.second ?? 0) / 1000,
  );
...
export function formatChartTickMark(time: Time, tickMarkType: TickMarkType): string {
  ...
  if (tickMarkType === TickMarkType.Year) return chartTickYearFormatter.format(displayDate);
  if (tickMarkType === TickMarkType.Month) return chartTickMonthFormatter.format(displayDate).replace(".", "");
  if (tickMarkType === TickMarkType.DayOfMonth) return chartTickDayFormatter.format(displayDate);
  if (tickMarkType === TickMarkType.TimeWithSeconds) return chartTickSecondsFormatter.format(displayDate);
...
export function timeframeBucketStart(unixSeconds: number, timeframe: string): number {
  ...
  if (timeframe === "M1") return Math.floor(Date.UTC(year, month, day, hour, minute, 0, 0) / 1000);
  if (timeframe === "M5") return Math.floor(Date.UTC(year, month, day, hour, Math.floor(minute / 5) * 5, 0, 0) / 1000);
  if (timeframe === "H1") return Math.floor(Date.UTC(year, month, day, hour, 0, 0, 0) / 1000);
  if (timeframe === "H2") return Math.floor(Date.UTC(year, month, day, Math.floor(hour / 2) * 2, 0, 0, 0) / 1000);
  if (timeframe === "H3") return Math.floor(Date.UTC(year, month, day, Math.floor(hour / 3) * 3, 0, 0, 0) / 1000);
  if (timeframe === "H4") return Math.floor(Date.UTC(year, month, day, Math.floor(hour / 4) * 4, 0, 0, 0) / 1000);
  if (timeframe === "D1") return Math.floor(Date.UTC(year, month, day, 0, 0, 0, 0) / 1000);
...
export function markerTimeToChartTime(markerTime: Time, timeframe: string, candleTimes: number[]): UTCTimestamp | null {
  ...
  if (candleTimes.includes(bucketTime)) return bucketTime as UTCTimestamp;

  const nearestTime = findNearestCandleTime(bucketTime, candleTimes);
  if (nearestTime === null) return null;

  const maxDistance = timeframeToSeconds(timeframe);
  if (Math.abs(nearestTime - bucketTime) > maxDistance) return null;
